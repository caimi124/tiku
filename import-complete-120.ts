...
console.log('\n' + '='.repeat(70));
console.log('🚀 2024年执业药师中药学综合知识与技能真题 - 完整导入（120道）');
console.log('='.repeat(70) + '\n');

// 完整120道题目数据（snake_case格式）
const allQuestions = [
  // 1-40题：最佳选择题
  ...
  {exam_type:'执业药师',subject:'中药学综合知识与技能',chapter:'中药贮藏',question_type:'single',content:'《中国药典》"凡例"中贮藏项下各名词术语进行了解释，关于中药贮藏名词说法，正确的是',options:[{key:'A',value:'密封系指将容器密闭，以防止尘及异物进入'},{key:'B',value:'遮光系指避免日光直射'},{key:'C',value:'阴凉处系指不超过20°C的环境'},{key:'D',value:'冷处系指0~8°C的环境'},{key:'E',value:'常温系指10~25°C的环境'}],correct_answer:'C',explanation:'阴凉处系指不超过20°C的环境，选项C说法正确。密封系指将容器密封，以防止风化，吸潮，挥发或异物进入，故A说法错误。遮光系指用不透光的容器包装，故B说法错误。冷处系指2~10°C的环境，故D说法错误。常温系指10~30°C的环境，故E说法错误。',difficulty:2,knowledge_points:['中药贮藏','药典知识'],source_type:'历年真题',source_year:2024},
...
async function main() {
  try {
    ...
    console.log(`📦 开始导入 ${allQuestions.length} 道题目...\n`);
    ...
    for (let i = 0; i < allQuestions.length; i++) {
      try {
        await prisma.questions.create({ data: allQuestions[i] });
        successCount++;
        const progress = `[${i + 1}/${allQuestions.length}]`;
        const percent = ((i + 1) / allQuestions.length * 100).toFixed(1);
        console.log(`✅ ${progress} (${percent}%) ${allQuestions[i].content.substring(0, 35)}...`);
      } catch (error: any) {
        errorCount++;
