 ...
 * 知识点详情页
 * 
 * 显示知识点完整信息：药物名称、重要性、掌握度、内容、记忆口诀
 * 
      ...
      <ol className="flex flex-wrap items-center gap-2 text-sm text-gray-500">
        ...
        {items.map((item, index) => (
          <li key={item.id} className="flex items-center gap-2">
            <span>/</span>
            <Link 
              href={`/knowledge?chapter=${item.id}`}
...
function ContentSection({ 
  ...
  const getAccuracyStyle = (accuracy: number) => {
    if (accuracy >= 80) return { icon: '✓', color: 'text-green-600', bg: 'bg-green-100' }
    if (accuracy >= 60) return { icon: '⚠', color: 'text-yellow-600', bg: 'bg-yellow-100' }
